...
nivel_2::nivel_2(){
	...
	ve.emplace_back(m_tex_enemy);
	ve[6].setPosition(250,275 );
	
	ve.emplace_back(m_tex_enemy);
	ve[7].setPosition(350, 220);
...
void nivel_2::update(juego &j){
	...
	if(m_ovni!=nullptr && m_ovni->debeDisparar()){
		...
		d.update();
		if (outofscreen_2(d)) {
			auto it = remove_if(vd.begin(), vd.end(), outofscreen_2);
	...
	for(disparo &d:vd_ovni){
		d.update_ovni();
		if (outofscreen_2(d)) {
			auto it = remove_if(vd_ovni.begin(), vd_ovni.end(), outofscreen_2);
			cout << "se elimino disparo fuera de pantalla " << endl;
	...
	for (size_t i = 0; i < vd.size(); ++i) {
		if(m_ovni != nullptr && colisiona_2(vd[i], *m_ovni)) {
			disparos_a_eliminar.push_back(i);
			m_ovni->RestarVida();
			if(m_ovni->VerVida() == 0){
				delete m_ovni;
	...
	for(auto it = enemigos_a_eliminar.begin(); it != enemigos_a_eliminar.end(); ++it) {
		eliminarEnemigo_2(ve, *it);
	}
	if(ve.empty() && m_ovni == nullptr) {
		m_sonido.stop();
		j.setScene(new escenaIntermedia_2());
	}
	m_n.update();
...
void nivel_2::draw(RenderWindow &window){				
	window.clear({0,0,0});
